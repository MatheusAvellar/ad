...
def remove_pending(events, content, time):
    ''' Remove requisições pendentes do conteudo `content`

    Parametros:
        events (list): lista de eventos (time, (cache, cid, content, timeout))
        content (str): conteudo que vai ser adiantado
    '''
    service_times = []
    for i in range(len(events)):
        _, (cache, cid, target_content, timeout) = events[i]
...
def simulate_cenario3(CacheType):
    events = []
    means_response_time = []
    hq.heapify(events)
    ...
    cache1 = CacheType(1, initial_contents, size=cache_size, theta=theta, mu=mu)
    cache2 = CacheType(2, initial_contents, size=cache_size, theta=theta, mu=mu)
